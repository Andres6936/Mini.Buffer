...
extern void
stretch_image(uint32_t* srcImage, uint32_t srcX, uint32_t srcY, uint32_t srcWidth, uint32_t srcHeight,
		uint32_t srcPitch,
...
mfb_open_ex(const char* title, unsigned width, unsigned height, unsigned flags)
{
	...
	if (flags & WF_BORDERLESS)
	{
		...
		XChangeProperty(window_data_x11->display, window_data_x11->window, sh_p, sh_p, 32, PropModeReplace,
				(unsigned char*)&sh, 5);
	}
	...
	if (flags & WF_ALWAYS_ON_TOP)
	{
		...
		XChangeProperty(window_data_x11->display, window_data_x11->window,
				XInternAtom(window_data_x11->display, "_NET_WM_STATE", False), XA_ATOM, 32, PropModeReplace,
				(unsigned char*)&sa_p, 1);
	}
	...
	if (flags & WF_FULLSCREEN)
	{
		...
		XChangeProperty(window_data_x11->display, window_data_x11->window,
				XInternAtom(window_data_x11->display, "_NET_WM_STATE", True), XA_ATOM, 32, PropModeReplace,
				(unsigned char*)&sf_p, 1);
	}
...
static int translateKeyCodeA(int keySym)
{
	...
	case XK_slash:
		return KB_KEY_SLASH;
	case XK_less:
		return KB_KEY_WORLD_1; // At least in some layouts...
	default:
		break;
	}
