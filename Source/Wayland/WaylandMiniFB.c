...
static void
pointer_enter(void* data, struct wl_pointer* pointer, uint32_t serial, struct wl_surface* surface, wl_fixed_t sx,
		wl_fixed_t sy)
{
	//kUnused(pointer);
	//kUnused(serial);
...
registry_global(void* data, struct wl_registry* registry, uint32_t id, char const* iface, uint32_t version)
{
	...
	{
		window_data_way->compositor = (struct wl_compositor*)wl_registry_bind(registry, id, &wl_compositor_interface,
				1);
	}
...
mfb_open_ex(const char* title, unsigned width, unsigned height, unsigned flags)
{
	...
	wl_surface_attach(window_data_way->surface, (struct wl_buffer*)window_data->draw_buffer, window_data->dst_offset_x,
			window_data->dst_offset_y);
