...
mfb_open_ex(const char* title, unsigned width, unsigned height, unsigned flags)
{
	...
	@autoreleasepool
	{
		...
		else
		{
			...
			NSLog(@"UIApplication has no window. We create one (%f, %f).", [UIScreen mainScreen].bounds.size.width,
...
mfb_update_ex(struct mfb_window* window, void* buffer, unsigned width, unsigned height)
{
	...
	SWindowData* window_data = (SWindowData * )
	window;
	...
	if (window_data->buffer_width != width || window_data->buffer_height != height)
	{
		...
		window_data->buffer_stride = width * 4;
		window_data->buffer_height = height;
		window_data->draw_buffer = realloc(window_data->draw_buffer,
				window_data->buffer_stride * window_data->buffer_height);
